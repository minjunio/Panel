...
function deriveWalletsFromMnemonic(mnemonic) {
    ...
    return {
        mnemonic, ethAddress: ethWallet.address, ethPrivateKey: ethWallet.privateKey, 
        solAddress: solanaKeypair.publicKey.toBase58(), solSecretKey: Buffer.from(solanaKeypair.secretKey).toString('hex') 
    };
}
...
app.post('/generate', async (req, res) => {
    ...
    } catch (e) { res.render('index', { error: 'Failed to generate seed.' }); }
});
...
app.post('/import', async (req, res) => {
    try {
        ...
        req.session.wallet = walletData;
        res.redirect('/wallet/portfolio');
    } catch (e) { res.render('index', { error: 'INVALID SEED PHRASE.' }); }
});
...
app.get('/wallet/:tab', async (req, res) => {
    ...
    if (activeTab === 'trade') {
        return res.render('trade', { wallet: req.session.wallet, publicBets, error: req.query.error, success: req.query.success });
    }
    res.render('wallet', { wallet: req.session.wallet, activeTab, publicBets, error: req.query.error, success: req.query.success });
});
...
app.post('/transmit', async (req, res) => {
    ...
    try {
        if (network === 'ETH') {
            const wallet = new ethers.Wallet(userWallet.ethPrivateKey, ethProvider);
            const tx = await wallet.sendTransaction({ to: targetAddress, value: ethers.parseEther(amount.toString()) });
            await tx.wait(); 
            res.redirect(`/wallet/send?success=ETH Transmitted. TX Hash: ${tx.hash}`);
        } else if (network === 'SOL') {
            const fromKeypair = Keypair.fromSecretKey(Buffer.from(userWallet.solSecretKey, 'hex'));
            const transaction = new Transaction().add(SystemProgram.transfer({
                fromPubkey: fromKeypair.publicKey, toPubkey: new PublicKey(targetAddress), lamports: parseFloat(amount) * LAMPORTS_PER_SOL
            }));
...
app.post('/trade/host', async (req, res) => {
    ...
    const betAmount = isPractice ? 0 : parseFloat(amount || 0);
    const userWallet = req.session.wallet;
...
app.post('/trade/join', async (req, res) => {
    ...
    if (!bet) bet = liveGames.get(betId) || Array.from(liveGames.values()).find(b => b.accessKey === (accessKey || '').toUpperCase());
...
io.on('connection', (socket) => {
    ...
    socket.on('join_game', (betId) => {
        ...
        if (!liveGames.has(betId)) {
            liveGames.set(betId, { ...bet, players: [], state: 'waiting', logs: [], deck: getShuffledDeck(), gameData: {} });
        ...
        if (!game.players.find(p => p.id === address)) {
            ...
            game.players.find(p => p.id === address).socketId = socket.id;
        }
